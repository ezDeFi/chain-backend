...
//
// Output {Consumer}
function updateConsumer({key, filter, applyLogs, mongoose, mongoosePrefix, maxPastRange}) {
    // reset the state
    // LogsStateModel.deleteOne({ key }).then(console.error).catch(console.error)

    maxPastRange = maxPastRange ?? 4

    let LogsStateModel = mongoose.model((mongoosePrefix ?? '') + 'LogsState')
    const processLogs = async ({ logs, fromBlock, toBlock, lastHead, head, getblockTimestamp  }) => {
    ...
    return {
        ...
        getRequests: async ({maxRange, lastHead, head}) => {
            ...
            const from = Math.max(state.range || lastHead, head ? head - _.min([maxRange, maxPastRange]) : 0) + 1
