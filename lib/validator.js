...
function _validateEthersProvider(provider) {
    if (!provider || provider?.constructor?.name !== 'JsonRpcProvider' && provider?.constructor?.name !== 'AssistedJsonRpcProvider') {
        throw new ChainBackendError(
            'invalid configuration "ethersProvider"'
        )
    }
}
function _validateAssistedJsonRpcProvider(provider) {
    return provider?.constructor?.name == 'AssistedJsonRpcProvider'
}
...
module.exports = {
    ...
    _validateAssistedJsonRpcProvider
}
