...
async function getBlockTimestamp(number) {
    ...
    const point = Math.ceil(new Date().getTime() / 1000) - 80000
    let range =  configs.blockTimeStampInterpolatingRange
    if (blockTimestamps.has(number)) {
        let time = blockTimestamps.get(number)
        if (point < time && time < now - 120) {
            console.log('!!!!!!!!!',point, time, now)
            console.log('number', number);
        }
        if (time > now + 180) {
            console.log('FUTURE1', 'time', time, 'now', now, 'blocknumber', number)
        }
    ...
    if (nA >= safeHead) {
        console.warn('unsafe time anchor point', { nA, safeHead, head: configs.head, range })
    ...
    if (blockTimestamps.has(nA)) {
        ...
        tA = await mustGetBlockTimestamp(nA)
        let t = await mustGetBlockTimestamp(number)
        console.log({number, t})
        // return t
        console.log('mustGetBlockTimestamp', {nA, tA})
        blockTimestamps.set(nA, tA)
    }
    if (blockTimestamps.has(nB)) {
        tB = blockTimestamps.get(nB)
    } else {
        tB = await mustGetBlockTimestamp(nB)
        blockTimestamps.set(nB, tB)
        console.log('mustGetBlockTimestamp', {nB, tB})
    }

    const time = Math.floor(tA + ((tA - tB) / (nA - nB)) * (number - nA))
    if (point < time && time < now - 60) {
        console.log(tA, '+ ((',tA, '-', tB,') / (', nA, '-', nB,')) * (',number, '-', nA, ')')
        console.log('@@@@@@@@@@','point', point, 'time', time, 'now', now)
        console.log('number', number, 'nA', nA, 'nB', nB, 'tA', tA, 'tB', tB);
    }
    if (time > now + 60) {
        console.log('nB = ', number - number % range)
        console.log(number, '-', number % range);
        console.log(tA, '+ ((',tA, '-', tB,') / (', nA, '-', nB,')) * (',number, '-', nA, ')')
        console.log('FUTURE2', 'time', time, 'now', now)
        console.log('number', number, 'nA', nA, 'nB', nB, 'tA', tA, 'tB', tB);
    }
    // console.log(time)
    return time
