...
async function getBlockTimestamp(number) {
    ...
    let tA = blockTimestamps[nA] ?? (await configs.config.getProvider().getBlock(nA)).timestamp
    let tB = blockTimestamps[nB] ?? (await configs.config.getProvider().getBlock(nB)).timestamp
    blockTimestamps[nA] = tA
    blockTimestamps[nB] = tB
