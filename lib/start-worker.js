...
async function getBlockTimestamp(number) {
    ...
    let blockA = blockTimestamps[nA] ?? await configs.config.getProvider().getBlock(nA)
    let blockB = blockTimestamps[nB] ?? await configs.config.getProvider().getBlock(nB)
    blockTimestamps[nA] = blockA.timestamp
    blockTimestamps[nB] = blockA.timestamp

    return Math.floor(blockA.timestamp + ((blockA.timestamp - blockB.timestamp) / (blockA.number - blockB.number)) * (number - blockA.number))
}
