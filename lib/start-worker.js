...
//  * config.minConfirmations {number=0} The worker processes block at
//    "latest - minConfirmations" instead of latest block. This configuration
...
async function _startWorker(config) {
    ...
    provider.on('block', blockNumber => {
        context.head = Math.max(0, blockNumber - config.minConfirmations)
    })
