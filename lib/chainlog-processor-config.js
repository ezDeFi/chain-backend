...
function chainlogProcessorConfig({type, provider, size, concurrency, hardCap=4000, target=500}) {
    const getProvider = (seed) => {
        if (!Array.isArray(provider)) {
            return provider
        }
        const i = Math.floor(provider.length * (seed ?? Math.random()))
        // if (seed) {
        //     console.log('sticky seeded provider', {seed, i})
        // }
        return provider[i]
    }
    ...
    const getLogs = async ({ address, fromBlock, toBlock, topics }, concurrency = 1, seed) => {
    ...
    const _tryGetLogs = async (params, seed) => {
        const RETRY = 8
        for (let i = 0; true; ++i) {
            try {
                return await getProvider(seed).getLogs(params);
            } catch(err) {
                if (i < RETRY) {
