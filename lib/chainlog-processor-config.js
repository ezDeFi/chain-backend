...
function chainlogProcessorConfig({type, provider, size, concurrency, hardCap=4000, target=500}) {
    ...
    const getLogs = async (requests, range, logMinFreq, safeBlock) => {
        ...
        if (logMinFreq != 0 && logs.length == 0 && (requests[0].toBlock ?? requests[0].fromBlock + range) - requests[0].fromBlock > logMinFreq) {
            throw new Errorr('unable to get logs')
        }
        return logs
    }
