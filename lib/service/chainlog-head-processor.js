...
function chainlogHeadProcessor({configs, consumers, mongoose, mongoosePrefix, safeDepth, logMinFreq, getBlockTimestamp}) {
    ...
    const process = async (head) => {
        ...
        if (fetcher) {
            ...
            logs = await config.getLogs(capped, toBlock-fromBlock+1, logMinFreq)
        } else {
            ...
            if (isMerged) {
                ...
                logs = await config.getLogs(merged, toBlock-fromBlock+1, logMinFreq, safeBlock)
            } else {
        ...
        for (const consumers of groups) {
            await Bluebird.map(consumers, consumer => {
                ...
                return consumer.processLogs({ logs: consumerLogs, fromBlock, toBlock, lastHead, head, getBlockTimestamp })
            })
        }
