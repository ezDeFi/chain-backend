...
function chainlogHeadProcessor({configs, consumers, mongoose}) {
    ...
    const process = async (head) => {
        ...
        console.log('++++ HEAD', { lastHead, behind: head-lastHead-1 })
        ...
        if (isMerged) {
            ...
            if (merged) {
                ...
                console.log('     MERGE:', { fromBlock, range: toBlock-fromBlock+1, behind: head-toBlock })
            ...
            console.log('     PARTN:', { fromBlock, range: toBlock-fromBlock+1, behind: head-toBlock })
