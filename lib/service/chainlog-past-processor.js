    ...
    return {
        ...
        process: async () => {
            ...
            if (fetcher) {
                ...
                logs = await Bluebird.map(chunks, async ({ fromBlock, toBlock }) => {
                    // partition similar requests base on the same combination of address and each topic
                    const merged = parts.map(requests => mergeRequests({requests, fromBlock, toBlock}))
                    if(merged[0]){
                        console.log('merged', parseInt(merged[0].fromBlock),parseInt(merged[0].toBlock) )
                    } else {
                        console.log('merged',undefined)
                    }
                    let result = config.getLogs(merged)
                    console.log('log: ',(await result).length)
                    return result
