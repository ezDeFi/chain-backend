...
const chainlogPastProcessor = ({configs, consumers, mongoose, mongoosePrefix, getBlockTimeStamp}) => {
    ...
    return {
        ...
        process: async () => {
            ...
            let range = 10000
            let nx = 20702000
            let nB = Math.floor(nx / range) * range
            let nA = nB + 1

            let blockA = await configs.partition.getProvider().getBlock(nA)
            if (blockA == null) {
                nA = nB
                nB = nA-1
                blockA = await configs.partition.getProvider().getBlock(nA)
            }
            let blockB = await configs.partition.getProvider().getBlock(nB)
            const interpolatedBlockTimestamps = (blockA, blockB, nx) => {
                return Math.floor(blockA.timestamp + ((blockA.timestamp - blockB.timestamp) / (blockA.number - blockB.number)) * (nx - blockA.number))
            }
            console.log('@@@@',interpolatedBlockTimestamps(blockA, blockB, nx))
