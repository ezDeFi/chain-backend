...
function createIndexedLogsFetcher(indexerAPI, config) {
    const getLogs = async (filter) => {
        ...
        for (let i = 0; i < MAX_RETRIES; ++i) {
            try {
                ...
                if (!['TIMEOUT', 'TOO GENERIC', 'SERVER_ERROR', 'NETWORK_ERROR'].includes(err.code)) {
                    console.error(err)
                }
    ...
    return {
        fetchLogs: async ({filters, fromBlock, toBlock}) => {
            ...
            for (const filter of filters) {
                for (const range of ranges) {
                    const f = {
                        fromBlock: _.max([range.from, filter.fromBlock, fromBlock]),
                        toBlock: _.min([range.from+range.range-1, filter.toBlock, toBlock]), // range.range can be undefined
                        address: filter.address,
                        topics: filter.topics,
                    }
                    if (f.fromBlock > f.toBlock ||
                        range.from > f.toBlock ||
                        range.from+range+range-1 < f.fromBlock
                    ) {
                        console.error('-----', {f, filter, range, fromBlock, toBlock})
                        continue
                    }
